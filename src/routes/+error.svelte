...
<svelte:head>
	<title>
		{title(
			$t('common.errorCodes.title', {
				code: $page.status,
				title: $t(`common.errorCodes.${$page.status}.title`)
			})
		)}
	</title>
</svelte:head>
...
<style>
	.errorcode {
		@apply font-inception text-primary-500 text-[25vw] leading-[20vw];

		/* glow */
		text-shadow: 0 0 10px rgb(var(--color-primary-500));
	}
</style>
